...
function defaultrequestLoggerAuth({ req, clientIP, userSession }) {
  ...
  //else
  if ((0, _matchInDepth.default)({ req, clientIP, userSession }, _debug.debugWriteToLogServerRequestAuth)) {
    logLevel = 'info';
  }

  if (logLevel) {
    (0, _log.default)(logLevel, 'rb-appbase-server auth request', { req, clientIP, userSession });
  }
}
