...
function defaultRequestLoggerGraphQL({ req, clientIP, userSession }) {
  ...
  //else
  if ((0, _matchInDepth.default)({ req, clientIP, userSession }, _debug.debugWriteToLogServerRequestGraphQL)) {
    logLevel = 'info';
  }

  if (logLevel) {
    (0, _log.default)(logLevel, 'rb-appbase-server GraphQL request', { req, clientIP, userSession });
  }
}
