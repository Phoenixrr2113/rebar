...
if (envHost == null || typeof envHost !== 'string') throw new Error('Error: urb-appbase-webapp requires the environment variable HOST to be set');function getSessionIdFromRequest(req) {const UserToken1 = req.cookies.UserToken1 || req.headers.usertoken1;if (UserToken1) try {if (UserToken1.length > 10) {const decoded = _jwtSimple2.default.decode(UserToken1, process.env.JWT_SECRET);return _defaultPersister2.default.uuidFromString(decoded.session_id);}} catch (err) {// Do nothing. This most probably means an expired session, or
  ...
  const session_id = getSessionIdFromRequest(req);let a_UserSession = null;if (session_id) a_UserSession = await objectManager.getOneObject('UserSession', { id: session_id, UserSession_artifact_id: objectManager.siteInformation.artifact_id });const user_id = a_UserSession ? a_UserSession.UserSession_User_id : _defaultPersister2.default.uuidNull();const a_User = await objectManager.getOneObject('User', { id: user_id, User_artifact_id: objectManager.siteInformation.artifact_id });if (a_User) {objectManager.setViewerUserId(user_id);return { User: a_User, UserSession: a_UserSession };} else {throw new Error('User ' + JSON.stringify(user_id) + ' not found');}}function verifyUserAuthToken(a_User, req) {if (!a_User) return Promise.reject('User not found');else {const request_UserToken2 = req.get('UserToken2');if (request_UserToken2 === a_User.UserToken2 || // A request coming from webapp will come from localhost and will bear the server's user token
    (req.ip === '127.0.0.1' || req.ip === envHost) && request_UserToken2 === _UserToken2ServerRendering2.default || // For use with GraphiQL
  ...
  var ip = req.headers['x-real-ip'] || req.connection.remoteAddress;const requestDetails = { headers: req.headers, cookies: req.cookies, ip: ip, query: req.body };_log2.default.log(
  'warn',
