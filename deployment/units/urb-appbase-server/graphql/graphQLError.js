...
maskErrors = maskErrors;var _graphql = require('graphql');var _log = require('../../urb-base-server/log');var _log2 = _interopRequireDefault(_log);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // Mark field/type/schema
...
class UserError extends Error {// $AssureFlow
  constructor(...args) {super(...args);this.name = 'Error';this.message = args[0]; // $AssureFlow
    this[IsUserError] = true; // $AssureFlow
    Error.captureStackTrace(this, 'Error');}}exports.UserError = UserError; // Modifies errors before sending to the user
...
function maskErrors(thing, fn = defaultHandler) {if (thing instanceof _graphql.GraphQLSchema) {maskSchema(thing, fn);} else if (thing instanceof _graphql.GraphQLObjectType) {maskType(thing, fn);} else {maskField(thing, fn);}}function maskField(field, fn) {const resolveFn = field.resolve;if (field[Processed] || !resolveFn) {return;}
