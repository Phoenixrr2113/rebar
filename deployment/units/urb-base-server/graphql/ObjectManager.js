...
const deletedRecord = { deleted: true // Regular expressions for detecting use of artifact_id and user_id in GraphQL Object Type
...
class ObjectManager {constructor() {// Loaders for a single record, by entity name
    ...
    this.siteInformation = { artifact_id: 'Object Manager: artifact_id has not been set' };}static registerEntity(entityName, EntityType, persister) {if (entityName in entityDefinitions) throw new Error('Entity already registered: ' + entityName); // In order to be able to access the name as a static property of the type
    ...
    const loaderIdentifier = Object.keys(query).sort().join(',');const loader = this.getLoader(entityName, loaderIdentifier, false);return loader.load(query).then(result => {const changes = this.changes[entityName];if (changes) {// $FlowIssue - by convention all entity objects are expected to have an id
        const change = changes[result.id];if (change != null) {if (change === deletedRecord) result = null; // Object is not found, return null // Add or update
          else Object.assign(result, change);}}return result;});}getObjectList(entityName, query) {// Apply artifact_id, User_id security
